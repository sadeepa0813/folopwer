...
function generateTrackingId(productId, customerName) {
    ...
    const random3 = Math.floor(100 + Math.random() * 900); // 3 digit random
    
    // Format: FP-XXXXXX-XXX (e.g., FP-123456-789)
    const trackingId = `FP-${timestamp}-${random3}`;
    ...
    try {
        ...
        if (error) {
            ...
            if (error.message && error.message.includes('row-level security')) {
                ...
                errorMessage += 'Duplicate tracking ID. Please try again.';
        ...
        // Show success modal
        showSuccessModal(data || orderData);
